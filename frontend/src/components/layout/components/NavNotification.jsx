...
export default function NavNotification() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'Iuran Belum Dibayar', message: 'Rumah A-12 belum bayar Satpam.', type: 'warning', time: '10m lalu', link: '/transaksi/iuran' },
    { id: 2, title: 'Pengeluaran Baru', message: 'Perbaikan selokan Rp 150.000.', type: 'info', time: '1j lalu', link: '/transaksi/pengeluaran' }
  ]);
            ...
            {notifications.length > 0 ? (
              notifications.map((n) => (
                <div
                  ...
                  className="relative flex items-start p-4 border-b border-gray-50 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/50 cursor-pointer group"
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 shrink-0 ${n.type === 'warning' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'}`}>
                  ...
                  <button onClick={(e) => dismissNotification(n.id, e)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100">
                    <HiXMark size={16} />
                  </button>
                </div>
              ))
