                  ...
                  <div key={`pay-${pay.id || idx}`} className="p-3 rounded-lg border border-slate-100 dark:border-slate-700/60 bg-white dark:bg-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                          pay.status === 'Lunas' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400' : 'bg-rose-100 text-rose-600'
                        }`}>
                        {pay.status === 'Lunas' ? <HiOutlineCheckCircle size={18} /> : <HiOutlineXCircle size={18} />}
                      </div>
                      <div>
                        <div className="text-xs font-bold text-slate-800 dark:text-slate-200">{pay.bulan}</div>
                        <div className="text-[11px] text-slate-400">Penghuni: <span className="font-medium text-slate-600 dark:text-slate-300">{pay.penghuniSaatItu}</span></div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs font-bold text-slate-800 dark:text-slate-200">Rp {pay.nominal.toLocaleString('id-ID')}</div>
                      <Badge variant={pay.status === 'Lunas' ? 'success' : 'danger'}>{pay.status}</Badge>
                    </div>
                  </div>
                )) : (!isPembayaranLoading && <p className="text-xs text-slate-400 text-center py-4">Belum ada riwayat transaksi iuran.</p>)}

                {/* Skeleton Pembayaran (Ditampilkan saat fetch data) */}
                  ...
                  <div key={`sk-pay-${i}`} className="p-3 rounded-lg border border-slate-100 dark:border-slate-700/60 bg-white dark:bg-slate-800 flex items-center justify-between animate-pulse">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="h-2 w-28 bg-slate-200 dark:bg-slate-700 rounded"></div>
                      </div>
                    </div>
